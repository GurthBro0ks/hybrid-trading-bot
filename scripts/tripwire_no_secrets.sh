...
if command -v rg &> /dev/null; then
    ...
    MATCHES=$(rg "$PATTERNS" . \
        ...
        --line-number | grep -vE "FailureReason|token_id|clobTokenIds|token_suffix|TOKEN_ID|TOKEN_IDS|OUTCOME_TOKEN")
    EXIT_CODE=$?
    set -e
    ...
    MATCHES=$(grep -E -r "$PATTERNS" . \
        ...
        -n | grep -vE "FailureReason|token_id|clobTokenIds|token_suffix|TOKEN_ID|TOKEN_IDS|OUTCOME_TOKEN")
    EXIT_CODE=$?
    set -e
